...
final class Session
{
    ...
    public static function start($Module = null)
    {
        if (!session_id()) {
            ...
            // Definir valores padrão se config não existir
            if (!$config) {
                $config = ['name' => 'banca_esportiva', 'sessiontime' => 180];
            }

            self::$_SESSION_ = $config['name'] ?? 'banca_esportiva';
            ...
            session_cache_expire($config['sessiontime'] ?? 180);
